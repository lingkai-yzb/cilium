...
func TestRingBufferReadNewCancel(t *testing.T) {
	forEachCapacity(t, testCapacities, nil, func(t *testing.T, capacity int, b *RingBuffer) {
		...
		select {
		case <-ch:
			// t.Fatal("unexpected receive") // FIXME
		default:
		}
...
func TestRingBufferReadSince(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			b := NewRingBuffer(WithCapacity(4))
			events := newLazyEventStream(t)
			events.fill(b)
			ch, cancel := b.ReadSince(events.time(tc.i), 8)
			events.fill(b)
			eventsReceived := 0
			for i := 0; i < tc.expected; i++ {
				...
				case <-ch:
					eventsReceived++
				default:
					t.Fatalf("%d events received, expected %d", eventsReceived, tc.expected)
				}
			}
			cancel()
			select {
			...
			assert.Equal(t, tc.expected, eventsReceived)
		})
	}
}
