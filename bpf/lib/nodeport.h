...
int tail_nodeport_ipv6_dsr(struct __ctx_buff *ctx)
{
	...
	if (!revalidate_data(ctx, &data, &data_end, &ip6))
		return DROP_INVALID;
#if DSR_ENCAP_MODE == DSR_ENCAP_IPIP
	addr.p1 = ctx_load_meta(ctx, CB_BE_ADDR_V6_1);
	addr.p2 = ctx_load_meta(ctx, CB_BE_ADDR_V6_2);
	addr.p3 = ctx_load_meta(ctx, CB_BE_ADDR_V6_3);
	addr.p4 = ctx_load_meta(ctx, CB_BE_ADDR_V6_4);
...
redo_local:
	...
	if (!backend_local) {
		...
		if (nodeport_uses_dsr6(&tuple)) {
#if DSR_ENCAP_MODE == DSR_ENCAP_IPIP
			ctx_store_meta(ctx, CB_BE_ADDR_V6_1, tuple.daddr.p1);
			ctx_store_meta(ctx, CB_BE_ADDR_V6_2, tuple.daddr.p2);
			ctx_store_meta(ctx, CB_BE_ADDR_V6_3, tuple.daddr.p3);
			ctx_store_meta(ctx, CB_BE_ADDR_V6_4, tuple.daddr.p4);
#elif DSR_ENCAP_MODE == DSR_ENCAP_NONE
...
redo_local:
	...
	if (!backend_local) {
		...
		if (nodeport_uses_dsr4(&tuple)) {
#if DSR_ENCAP_MODE == DSR_ENCAP_IPIP
			ctx_store_meta(ctx, CB_BE_ADDR_V4, tuple.daddr);
#elif DSR_ENCAP_MODE == DSR_ENCAP_NONE
			ctx_store_meta(ctx, CB_SVC_PORT, key.dport);
